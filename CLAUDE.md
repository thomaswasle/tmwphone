...
- `SettingsDialog` has a "Quickdial" preferences page (`build_quickdials_ui` / `make_quickdial_row`) with per-entry add/edit/delete and up/down reorder (`reorder_quickdial` swaps neighbours and rebuilds the group), following the accounts-page pattern (reload list → mutate by `id` → save). The saved list order is the sidebar render order.
